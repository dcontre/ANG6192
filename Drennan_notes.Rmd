...
subtitle: "Working Notes for ANG6192/ANT4117"
...
These notes, intended to serve as a guide to implementing Drennan's *Statistics for Archaeologists* [@Drennan_2009] in R, are built upon David Carlson's [*R Companion to Statistics for Archaeologists*](https://sites.google.com/a/tamu.edu/dlcarlson/home/r-project/companion-guides). I have not used Carlson's text, and I have rewritten most of the code, but without the foundation - and inspiration - that he provided this document would not exist.

```{r echo = F, out.width= "35%", out.extra='style="float:right; padding:10px"'}
include_graphics("imgs/drennan.jpg")
```

Note that there is now a Spanish-language edition of Drennan's book available. If you'd like these notes in Spanish as well (or another language, for that matter), paste the url into [Google Translate](https://translate.google.com/?sl=es&tl=en&op=websites) or your LLM of choice.  

```{r echo = F, out.width= "35%", out.extra='style="float:right; padding:10px"'}
include_graphics("imgs/drennan_espanol.png")
```
...
Download ['scrapers.txt'](data/Drennan_datasets/scrapers.txt). Often you'll want to read an existing table (.xls, .csv, .txt, etc.) into R. Get used to figuring out where files live on your hard drive, and learn how to access the filepath (shift + rightclick on Windows; Option + Command + C on Mac). 
...
You'll use `read.delim()` for tab-delimited; `read.table()` or `read.csv()` are comparable for other data structures. (There are R packages that also enable reading data with more complex formats - Excel tables or Google sheets, most commonly.). Get used to examining the structure of any data file so that you can figure out how to use it (and about what specifically to curse the original author if necessary).  There are various tricks for this - select + spacebar on a Mac, or TextEdit (included w/ the OS; [TextWrangler and its spawn, BBEdit](https://www.barebones.com/products/bbedit/) are free and better); on a PC try the free utility [Notepad++](https://notepad-plus-plus.org/).
Using - not to mention producing and maintaining - tabular data is a generally-overlooked but vital part of archaeological practice. One important advantage of working with data in R is that you can leave the original data file alone, instead making modifications to the objects you're working with in R. This removes the risk - endemic to spreadsheet use - of corruption or loss of original data.  Warning: making active use of tabular data will lead to you develop opinions about how one *should* manage such data (and there are in fact better and worse practices; for spreadsheets in particular see Broman and Woo [-@broman2018]). For notes on wrangling data in R, see [here](https://dcontre.github.io/DataWrangling_notes/introduction.html).  
...
```{r echo = F, out.width= "35%", out.extra='style="float:right; padding:10px"'}
include_graphics("imgs/homer_brain.jpg")
```
...
## Comparison with $\chi^2$ {#chi-square}
...
While linear regression is very commonly used, count data are better approached through generalized linear models (GLMs) [@Codding2024; @mccall2024]. Drennan does not address these, but see (here)[###addd link###]).
...
Drennan refers to Table 22.6 as a cross-tabulation. These are often also called 'contingency tables', and can consist of counts rather than simple presence/absence data; these are the basis of [($\chi^2$ analysis](#chi-square), though Drennan does not describe them in such terms. Contingency tables are the basis of other multivariate techniques, and we will discuss them in more detail when we cover [correspondence analysis](#correspondence-analysis). For more on count data and contingency tables, see [here](###add link###). 
...
![](imgs/Shennan_Table13-8.jpg)


The full table is [here](data/HoardsWide.RData), in '.Rdata' format, which you can load directly into R. As you can imagine, the resulting CA table has a lot going on. 
...
Complex analyses can be vital means of developing insights and testing ideas, but often simple reorganization and visualization of data are at least as useful. Particularly for the former, *tidyverse* tools are efficient and flexible (if sometimes reliant on syntax that is difficult to decipher). Even as you add additional analytical methods to your toolbox, you may want to revisit the notes on [Data Wrangling for Archaeology](https://dcontre.github.io/DataWrangling_notes/introduction.html) and plotting (using both [Base R](###add link###) and [ggplot](###add link###)). There are *many* guides available to using R for data visualization, and a few specifically archaeological treatments (e.g., [@banning2020_viz; @Baxter2016graphics]; the latter is available [here](http://www.barbicanra.co.uk/simple-r.html)). See also Joe Roe's slides on [*Data Visualization for Archaeologists in R*](https://joeroe.io/r4r_visualisation/r4r_visualisation_slides.html#1). 

## Other analytical approaches

Correspondence analysis is, of course, only one of many methods that don't make it into Drennan's book. For a brief review of books on quantitative methods in archaeology, see Drennan's section on 'Suggested Reading' [-@Drennan_2009, 321-325]. More recent texts not covered there include include the 2nd edition of Baxter's *Exploratory multivariate Analysis in Archaeology* [-@baxter2015exploratory], VanPool and Leonard's *Quantitative Analysis in Archaeology* [-@VanPool_Leonard_2011], McCall's *Strategies for Quantitative Research: Archaeology by Numbers* [-@mccall2018strategies], Carlson's *Quantitative methods in Archaeology Using R* [-@carlson2017quantitative], and the first half of Banning's *The Archaeologist’s Laboratory* [-@banning2020, Ch.1-8]. Baxter's *Notes on Quantitative Archaeology and R* [-@Baxter2015quantarchR], available [here](https://www.researchgate.net/publication/277931925_Notes_on_Quantitative_Archaeology_and_R) though not formally published, is at least as cogent and useful as many things that are.

There are several recent chapters and papers presenting introduction to particular topics, especially multivariate [@glascock2023; @lopezgarcia2026], Bayesian [@OtarolaCastillo2018; @OtarolaCastillo2022], and modeling [@crema2025] approaches. There are even more examples of their application, particularly in spheres where analytical approaches are fundamental to data interpretation (for example in use of chemical compositional data); listing those is beyond what I can do here. 
Attention to these topics results at least in part from the increase in easily-available computational power that we touched on when we covered [resampling approaches](#resampling). They also tend to reflect the reservations about null-hypothesis statistical testing (NHST) and the skepticism about fetishization of p-values that runs through Drennan (though they certainly do not originate with him [see @Cowgill1977], and are becoming common in the sciences more broadly [see @Amrhein2019; @wasserstein2019]).
